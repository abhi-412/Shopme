...
return (
  <div onClick={()=>{handleSelect(order?._id)}} className=" p-3 mb-3 flex justify-between items-center hover:shadow-md hover:scale-105 cursor-pointer transition delay-75 duration-300 border-b border-gray-600 ">
    ...
    {products?.map((p)=>{
        return <div key={p.product?._id} className='flex justify-between gap-1'>
            ...
            <div className='flex flex-col gap-1 justify-center text-gray-600 text-sm'>
                <p className='text-sm text-black'><span className={` text-sm font-medium ${order?.orderStatus === 'Delivered' ? ' text-green-600' : order?.orderStatus === 'Cancelled' ? ' text-red-600' : ' text-yellow-500'}`}>{order?.orderStatus}</span> on {new Intl.DateTimeFormat('en-US', { month: 'long', day: '2-digit' }).format(new Date(order?.updatedAt))}</p>
